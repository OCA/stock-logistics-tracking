        ...
        for pack in pack_ids:
            ...
            if pack.parent_id:
                if pack.parent_id and pack.parent_id != 'open':
                    self.write(
                        cr, uid, [pack.parent_id.id], {'state': 'open'}, context=context)
            if allowed:
                for child in pack.child_ids:
                    if child.state == 'open':
                        allowed = False
                        raise osv.except_osv(_('Not allowed !'), _(
                            'You can\'t re-open this pack because there is at least one not closed child'))
                        break
            if allowed:
                self.write(
                    cr, uid, [pack.id], {'state': 'open'}, context=context)
        return True
...
class stock_move(orm.Model):
    ...
    _constraints = [
        (_check_tracking_state,
         'You cannot choose this pack because it\'s closed.', ['tracking_id']),
    ]
