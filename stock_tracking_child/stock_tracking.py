...
class stock_tracking(orm.Model):
    ...
    def _get_child_products(self, cr, uid, ids, field_name, arg,
                            context=None):
    ...
    _columns = {
        ...
        'child_ids': fields.one2many(
            'stock.tracking', 'parent_id',  'Children', readonly=True),
        'ul_id': fields.many2one('product.ul', 'Logistic unit'),
        'child_product_ids': fields.function(
            _get_child_products, method=True, type='one2many',
            obj='product.stock.tracking', string='Child Products'),
        'child_serial_ids': fields.function(
            _get_child_serials, method=True, type='one2many',
            obj='serial.stock.tracking', string='Child Serials'),
    }

    def _create_data_product(self, cr, uid, child, product_list,
                             context=None):
        if context is None:
            context = {}
    ...
    def get_products(self, cr, uid, ids, context=None):
        ...
        for pack in pack_ids:
            ...
            for child in childs:
                ...
                self._create_data_product(cr, uid, child, product_list,
                                          context=context)
        return True
    ...
    def get_serials(self, cr, uid, ids, context=None):
        ...
        for pack in pack_ids:
            ...
            for child in childs:
                ...
                self._create_data_prodlot(cr, uid, child, serial_list,
                                          context=context)
        return True
    ...
    def _check_parent_id(self, cr, uid, ids, context=None):
        ...
        for track in trackings:
            if track.parent_id and track.parent_id.ul_id and track.ul_id and \
                    track.ul_id.capacity_index > \
                    track.parent_id.ul_id.capacity_index:
                return False
        return True

    _constraints = [(_check_parent_id,
                     'Bad parent type selection. The UL is not well set.',
                     ['parent_id'])]
