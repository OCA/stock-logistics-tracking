...
class stock_tracking(orm.Model):
    ...
    def reset_open(self, cr, uid, ids, context=None):
        res = super(stock_tracking, self).reset_open(
    ...
    def set_close(self, cr, uid, ids, context=None):
        ...
        history_obj = self.pool.get('stock.tracking.history')
        res = super(stock_tracking, self).set_close(
            cr, uid, ids, context=context)
        if res:
            pack_ids = self.browse(cr, uid, ids, context=context)
            for pack in pack_ids:
                if pack.state == 'open':
                    if history_obj.search(cr, uid, [
                        ...
                        new_pack_id = self._create_pack(
                            cr, uid, pack.id, context=context)
                        new_pack_data = self.browse(
                            cr, uid, new_pack_id, context=context)
                        ...
                        if pack.child_ids:
                            for child_pack_data in pack.child_ids:
                                if child_pack_data.state == 'close':
                                    self.write(
                                        cr, uid, child_pack_data.id, {'active': False}, context=context)
                                    self.write(cr, uid, [new_child_pack_id], {
                                               'parent_id': new_pack_data.id, }, context=context)
                                    history_obj.create(cr, uid, {
                                        'type': 'pack_in',
                        ...
                        self.get_products(
                            cr, uid, [new_pack_data.id], context=context)
                        self.get_serials(
                            cr, uid, [new_pack_data.id], context=context)
                    self.write(
                        cr, uid, [pack.id], {'state': 'close'}, context=context)
        return res
...
class stock_tracking_history(orm.Model):
    ...
    def _get_types(self, cr, uid, context={}):
        res = super(stock_tracking_history, self)._get_types(
