...
class stock_fill_inventory(orm.TransientModel):
    ...
    def fill_inventory(self, cr, uid, ids, context=None):
        ...
        if stock_inventory_obj.browse(cr, uid, context.get('active_id', False), context).location_id:
            stock_inventory_obj.write(cr, uid, context.get(
                'active_id', False), {'location_id': fill_inventory.location_id.id})
        return res
...
class acquisition_acquisition(orm.Model):
    ...
    def onchange_destination(self, cr, uid, ids, destination_id=False, context=None):
        ...
        if destination_id:
            destination = self.pool.get(
                'stock.location').browse(cr, uid, destination_id)
    ...
    def onchange_inventory(self, cr, uid, ids, inventory_id=False, context=None):
        ...
        if inventory_id:
            inventory = self.pool.get(
                'stock.inventory').browse(cr, uid, inventory_id)
...
#                }
    ...
    def check_production_lot_location(self, cr, uid, origin_id, logistic_unit_id, context=None):
        ...
        stock_production_lot_data = self.pool.get(
            'stock.production.lot').browse(cr, uid, logistic_unit_id)
        logistic_unit_number = stock_production_lot_data.id
        product = stock_production_lot_data.product_id
        name_list = self.pool.get('product.product').name_get(
    ...
    def update_delivery_order_line(self, cr, uid, product_id=False, res_id=False, order_id=False, origin_id=False, destination_id=False, tracking_id=False, quantity=1, context=None):
        ...
        if product_id:
            ...
            if move_ids and not product_product:
                ...
                split_context.update({'active_id': move_ids[0], 'active_ids': [
                                     move_ids[0]], 'active_model': 'stock.move', 'tracking_id': tracking_id})
                split_id = split_obj.create(cr, uid, vals, split_context)
                split_line_obj.create(
                    cr, uid, {'prodlot_id': res_id, 'wizard_exist_id': split_id, 'quantity': quantity})
                ...
                name_list = product_obj.name_get(
    ...
    def process(self, cr, uid, ids, context=None):
        ...
        for acquisition in acquisition_data:
            ...
            self.write(
                cr, uid, acquisition.id, {'state': 'done'}, context=context)
        return res
    ...
    def order_preparation(self, cr, uid, acquisition, context=None):
        ...
        for line in acquisition.acquisition_ids:
            ...
            parent_id = self.update_stock_move(
                cr, uid, line, parent_id, context)
        return res
    ...
    def update_order(self, cr, uid, acquisition, context=None):
        ...
        stock_picking_obj.write(
            cr, uid, picking_id, {'address_id': address_id, 'type': 'out'})
        return picking_id
    ...
    def update_stock_move(self, cr, uid, line, parent_id=None, context=None):
        ...
        acquisition_setting_ids = setting_obj.search(
            cr, uid, [('barcode_id', '=', barcode.id)], limit=1)
        if acquisition_setting_ids:
            acquisition_setting_id = acquisition_setting_ids[0]
            acquisition_setting_data = setting_obj.browse(
                cr, uid, acquisition_setting_id, context)
            if acquisition_setting_data.action_type == 'create_pack':
                logistic_unit = self.pool.get('product.ul').search(
                    cr, uid, [('type', '=', 'pack')], limit=1)[0]
                parent_id = setting_obj.create_pack(
                    cr, uid, [line.acquisition_id.id], logistic_unit, context)
                return parent_id
        ...
        if res_model == 'stock.production.lot':
            ...
            self.check_production_lot_location(
                cr, uid, origin_id, res_id, context)
            ...
            if stock_tracking_data.parent_id:
                raise osv.except_osv(_('Warning!'), _(
                    'You cannot move this pack because it\'s inside of an other pack: %s.') % (stock_tracking_data.parent_id.name))
            for child in stock_tracking_data.child_ids:
                if child.state != 'close':
                    raise osv.except_osv(_('Warning!'), _(
                        'You cannot move this pack because there is a none closed pack inside of it: %s.') % (child.name))
        ...
        for line in acquisition.acquisition_ids:
            if first_code == True:
                ...
                order_id = self.create_order(
                    cr, uid, [acquisition.id], context)
                acquisition_obj.write(
                    cr, uid, acquisition.id, {'picking_id': order_id})
            if line.type == 'object':
                self.add_stock_move(
                    cr, uid, [acquisition.id], line, order_id, context)
            else:
                tracking_id = setting_obj.do_action(
                    cr, uid, [acquisition.id], line.type, context)
    ...
    def create_order(self, cr, uid, ids, context=None):
        ...
        '''order creation'''
        order_id = stock_picking_obj.create(
            cr, uid, {'address_id': address_id, 'type': 'out'})
...
#            self.write(cr, uid, acquisition_data.id, vals, context=context)
    ...
    def add_stock_move(self, cr, uid, ids, line, order_id, context=None):
        ...
        if barcode_data.res_model == 'stock.production.lot':
            stock_production_lot_data = stock_production_lot_obj.browse(
                cr, uid, logistic_unit_id)
            logistic_unit_number = stock_production_lot_data.id
            product = stock_production_lot_data.product_id
            name_list = self.pool.get('product.product').name_get(
                cr, uid, [product.id], context)
            stock_production_lot_name = name_list[0][1]
            '''If the production lot is not in the current stock'''
            new_move_id = self.check_production_lot_location(
                cr, uid, origin_id, logistic_unit_id, context)
            ''''stock move creation'''
            ...
            stock_tracking_data = stock_tracking_obj.browse(
                cr, uid, logistic_unit_id)
            if stock_tracking_data.parent_id:
                raise osv.except_osv(_('Warning!'), _(
                    'You cannot move this pack because it\'s inside of an other pack: %s.') % (stock_tracking_data.parent_id.name))
            ...
            for child_pack in child_packs:
                ...
                for move in child_pack.current_move_ids:
                    ...
                    new_id = stock_move_obj.copy(
                        cr, uid, move.id, default=defaults, context=context)
                    stock_move_obj.write(
                        cr, uid, [move.id], {'pack_history_id': hist_id, 'move_dest_id': new_id})

                stock_tracking_obj.write(
                    cr, uid, [child_pack.id], {'location_id': destination_id})
        '''End'''
        return res
    ...
    def pack_preparation(self, cr, uid, acquisition, context=None):
        ...
        for line in acquisition.acquisition_ids:
            if first_code == True:
                ...
                parent_id = setting_obj.create_pack(
                    cr, uid, [acquisition.id], logistic_unit, context)
...
#                    vals['name'] = name
            ...
            setting_obj.add_child(
                cr, uid, line.barcode_id.id, parent_id, context)
        if parent_id:
            setting_obj.close_pack(cr, uid, [parent_id], context)
        return res
    ...
    def inventory_preparation(self, cr, uid, acquisition, context=None):
        ...
        if not inventory_id:
            raise osv.except_osv(
                _('Warning!'), _('There is no inventory, please create one!'))
        if inventory_id.state != 'draft':
            raise osv.except_osv(
                _('Warning!'), _('This inventory is not in draft state, you can\'t continue !'))
        location_id = acquisition.origin_id or False
        if not location_id:
            raise osv.except_osv(
                _('Warning!'), _('There is no location defined, please select one!'))
        for line in acquisition.acquisition_ids:
    ...
    def check_inventory_line(self, cr, uid, line, context=None):
        ...
        if barcode.res_model == 'stock.production.lot':
            ...
            production_lot = production_lot_obj.browse(
                cr, uid, logistic_unit_id)
            product = production_lot.product_id
            ...
            line_ids = inventory_line_obj.search(cr, uid, [(
                'inventory_id', '=', inventory_id), ('product_id', '=', product.id), ('prod_lot_id', '=', lot_id)])
            if line_ids:
            ...
            line_ids = inventory_line_obj.search(cr, uid, [(
                'inventory_id', '=', inventory_id), ('product_id', '=', product.id), ('location_id', '=', location_id)])
            if line_ids:
                qty = inventory_line_obj.read(
                    cr, uid, line_ids[0], ['product_qty'])['product_qty']
                vals_update = vals
                vals_update.update(
                    {'product_qty': qty >= 0 and qty + line.quantity or line.quantity})
    ...
    def move_stock_preparation(self, cr, uid, acquisition, context=None):
        ...
        for line in acquisition.acquisition_ids:
            if first_code == True:
                first_code = False
                move_stock_id = self.create_move_stock(
                    cr, uid, [acquisition.id], context=context)
#                name = self.pool.get('stock.inventory').browse(cr, uid, move_stock_id, context=context).name or False
            self.add_stock_move_line(
                cr, uid, [acquisition.id], line, move_stock_id, context)
        if move_stock_id:
    ...
    def add_stock_move_line(self, cr, uid, ids, line, inventory_id, inventory_line_id, context=None):
        ...
        if barcode_data.res_model == 'stock.production.lot':
            stock_production_lot_data = stock_production_lot_obj.browse(
                cr, uid, logistic_unit_id, context=context)
            product = stock_production_lot_data.product_id
            logistic_unit_number = stock_production_lot_data.id
            vals = {
                'inventory_id': inventory_id,
...
#        return tracking_id
    ...
    def do_action(self, cr, uid, ids, action_type, context=None):
        ...
        if action_type == 'create_add':
            ...
            if tracking_to_close_id:
                self.close_pack(
                    cr, uid, [tracking_to_close_id], context=context)
        return tracking_id
    ...
    def create_pack(self, cr, uid, ids, ul_id=None, context=None):
        ...
        acquisition_data = self.pool.get(
            'acquisition.acquisition').browse(cr, uid, ids[0])
        location_id = acquisition_data.origin_id.id
        if ul_id == None:
            ul_id = self.pool.get('product.ul').search(cr, uid, [], limit=1)[0]
        logistic_unit = ul_id
        '''Pack Creation'''
        tracking_id = stock_tracking_obj.create(
            cr, uid, {'ul_id': logistic_unit, 'location_id': location_id})
    ...
    def close_pack(self, cr, uid, ids, context=None):
        ...
        if self.pool.get('acquisition.acquisition').browse(cr, uid, context.get('current_id'), context).picking_id:
            origin_id = self.pool.get('acquisition.acquisition').browse(
                cr, uid, context.get('current_id'), context).origin_id.id
            destination_id = self.pool.get('acquisition.acquisition').browse(
                cr, uid, context.get('current_id'), context).destination_id.id
            stock_move_ids = self.pool.get('stock.move').search(
                cr, uid, [('tracking_id', '=', ids[0])])
            for stock_move_id in stock_move_ids:
                stock_move_data = self.pool.get('stock.move').browse(
                    cr, uid, stock_move_id, context)
                self.pool.get('stock.move').write(cr, uid, stock_move_id, {
                    'state': 'done', 'picking_id': False, 'location_dest_id': origin_id}, context)
